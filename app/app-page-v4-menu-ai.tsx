...
export default function Home() {
  ...
  return <main className="app">
    <aside className="sidebar">
      ...
      <nav>{tools.map((tool) => { const Icon = tool.icon; return <button key={tool.id} type="button" className={active === tool.id ? "sideItem active" : "sideItem"} onClick={() => { setActive(tool.id); if (tool.id === "menu") document.getElementById("menu-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "ceo") document.getElementById("diagnosis")?.scrollIntoView({behavior:"smooth"}); else document.getElementById("ai")?.scrollIntoView({behavior:"smooth"}); }}><Icon size={18}/><span>{tool.name}</span>{active === tool.id && <i/>}</button>; })}</nav>
