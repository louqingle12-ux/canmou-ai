...
要求：中文、数字优先、短句、可执行，不假装知道真实损耗率。`;
  ...
  const dashboardRevenue = dailyReportResult ? money(dailyReportResult.revenue) : "¥8,620";
  const dashboardOrders = dailyReportResult ? dailyReportResult.orders.toLocaleString("zh-CN") : "386";
  const dashboardAov = dailyReportResult ? money(dailyReportResult.aov) : "¥22.33";
  const dashboardGrossProfit = dailyReportResult ? money(dailyReportResult.grossProfit) : "¥4,921";
  const dashboardLabel = dailyReportResult ? "已录入" : "演示数据";
  ...
  return <main className="app">
    ...
    <section className="content">
      ...
      <section className="panel" id="diagnosis" style={{marginBottom:24,padding:24,borderRadius:22}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:14,marginTop:20}}>
          {[
            ["revenue","今日营业额","例如 8620","元"],
            ["orders","今日订单","例如 386","单"],
            ["targetAov","目标客单价","例如 32","元"],
            ["foodCost","食材成本","可选","元"],
            ["commission","平台佣金","可选","元"],
            ["reviews","差评数","例如 3","条"],
          ].map(([key,label,placeholder,suffix]) => (
            <label key={key} style={{display:"block",position:"relative",zIndex:2}}>
              <span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:7,color:"#3e5149"}}>{label}</span>
              <div style={{display:"flex",alignItems:"center",border:"1px solid #dce6e1",borderRadius:12,background:"#fff",padding:"0 12px",position:"relative",zIndex:3}}>
                <input
                  type="text"
                  inputMode="decimal"
                  value={diag[key as keyof typeof diag]}
                  onChange={(e)=>updateDiag(key as keyof typeof diag,e.target.value)}
                  placeholder={placeholder}
                  autoComplete="off"
                  style={{width:"100%",height:46,border:0,outline:"none",background:"transparent",fontSize:15,color:"#14201b",cursor:"text",pointerEvents:"auto",touchAction:"manipulation"}}
                />
                <span style={{fontSize:12,color:"#8a9a93",whiteSpace:"nowrap"}}>{suffix}</span>
              </div>
            </label>
          ))}
        </div>
