...
export default function Home() {
  ...
  const [dailyReport, setDailyReport] = useState<DailyReport>({ revenue: "", orders: "", foodCost: "", commission: "", reviews: "", targetAov: "32" });
  const [dailyReportResult, setDailyReportResult] = useState<DailyReportResult | null>(null);
  const [dailyReportAnswer, setDailyReportAnswer] = useState("");
  const [dailyReportLoading, setDailyReportLoading] = useState(false);
  const [dailyHistory, setDailyHistory] = useState<DailyRecord[]>([]);
  const [historyLoading, setHistoryLoading] = useState(false);
  const [review7Answer, setReview7Answer] = useState("");
  const [review7Loading, setReview7Loading] = useState(false);
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (currentSession) { await loadCredits(currentSession); await loadDailyHistory(currentSession); } else setRemaining(5);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, newSession) => {
      ...
      if (newSession) { await loadCredits(newSession); await loadDailyHistory(newSession); } else { setRemaining(5); setDailyHistory([]); }
    });
  ...
  async function loadDailyHistory(currentSession?: any) {
    const current = currentSession || session;
    if (!current?.user?.id) { setDailyHistory([]); return; }
    setHistoryLoading(true);
    try {
      const { data, error } = await supabase
        .from("daily_operating_data")
        .select("id,user_id,record_date,revenue,orders,food_cost,commission,reviews,target_aov")
        .eq("user_id", current.user.id)
        .order("record_date", { ascending: false })
        .limit(30);
      if (error) {
        console.error("daily_operating_data load error", error);
        return;
      }
      const rows = (data || []) as DailyRecord[];
      setDailyHistory(rows);
      const today = rows.find(r => r.record_date === localDateString());
      if (today) {
        setDailyReport({
          revenue: String(today.revenue ?? ""),
          orders: String(today.orders ?? ""),
          foodCost: today.food_cost == null ? "" : String(today.food_cost),
          commission: today.commission == null ? "" : String(today.commission),
          reviews: String(today.reviews ?? ""),
          targetAov: String(today.target_aov ?? 32),
        });
      }
    } catch (e) {
      console.error(e);
  ...
  async function reviewLast7Days() {
    if (!dailyHistory.length || review7Loading) return;
    const { data: { session: currentSession } } = await supabase.auth.getSession();
    if (!currentSession) { setShowAuth(true); return; }
    if (remaining === 0) { setShowPro(true); return; }

    const rows = [...dailyHistory].sort((a,b) => a.record_date.localeCompare(b.record_date)).slice(-7);
    const totalRevenue = rows.reduce((s,r) => s + Number(r.revenue || 0), 0);
    const totalOrders = rows.reduce((s,r) => s + Number(r.orders || 0), 0);
    const avgAov = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    const profitRows = rows.filter(r => r.food_cost !== null && r.commission !== null);
    const totalProfit = profitRows.length ? profitRows.reduce((s,r) => s + Number(r.revenue || 0) - Number(r.food_cost || 0) - Number(r.commission || 0), 0) : null;
    const reviewCount = rows.reduce((s,r) => s + Number(r.reviews || 0), 0);
    const first = rows[0];
    const last = rows[rows.length - 1];
    const revenueChange = first && first.revenue > 0 ? ((last.revenue - first.revenue) / first.revenue) * 100 : null;

    const prompt = `你是餐谋AI的经营复盘专家。请根据老板最近${rows.length}天的真实经营数据做一次“AI自动复盘”。只允许使用提供的数据，不要编造同比、行业平均、流量、顾客反馈或原因。无法确认的原因必须写“需要进一步验证”。

${rows.map((r,i) => {
      const gp = r.food_cost !== null && r.commission !== null ? r.revenue - r.food_cost - r.commission : null;
      const aov = r.orders > 0 ? r.revenue / r.orders : 0;
      return `${i+1}. ${r.record_date}｜营业额¥${Number(r.revenue).toFixed(2)}｜订单${r.orders}｜客单价¥${aov.toFixed(2)}｜食材成本${r.food_cost == null ? "未提供" : "¥" + Number(r.food_cost).toFixed(2)}｜平台佣金${r.commission == null ? "未提供" : "¥" + Number(r.commission).toFixed(2)}｜毛利${gp == null ? "无法计算" : "¥" + gp.toFixed(2)}｜差评${r.reviews}`;
    }).join("\n")}

7天累计营业额：¥${totalRevenue.toFixed(2)}
7天累计订单：${totalOrders}
7天平均客单价：¥${avgAov.toFixed(2)}
7天可计算毛利：${totalProfit === null ? "数据不足" : "¥" + totalProfit.toFixed(2)}
7天差评：${reviewCount}
首尾营业额变化：${revenueChange === null ? "无法判断" : revenueChange.toFixed(1) + "%"}

严格按以下结构输出：
【7天一句话】一句话总结这7天最重要的经营变化。
【最值得肯定】最多2条，只根据数据判断。
【最大问题】指出最值得解决的1个问题，并说明数据依据。
【可能原因】最多3条；没有数据支持的原因必须标注“待验证”。
【钱从哪里来】给出1个最现实的增收或增利机会，明确是策略建议还是估算。
【下周3个动作】按优先级给出3个具体动作。
【下周盯3个数字】列出3个指标和目标关注方向。
要求：中文、短句、直接、数字优先，不说鸡汤。`;

    setReview7Loading(true);
    setReview7Answer("");
    try {
      ...
      if (!res.ok) throw new Error(data?.error || "AI复盘失败，请稍后再试。");
      setReview7Answer(data?.answer || "AI没有返回有效内容。");
      if (typeof data?.remaining === "number") setRemaining(data.remaining); else await loadCredits(currentSession);
    } catch (e:any) { setReview7Answer(e?.message || "请求失败，请稍后再试。"); }
    finally { setReview7Loading(false); }
  }

  async function saveDailyReport(currentSession: any, values: DailyReport) {
    const recordDate = localDateString();
    const payload = {
      user_id: currentSession.user.id,
      record_date: recordDate,
      revenue: Number(values.revenue) || 0,
      orders: Number(values.orders) || 0,
      food_cost: values.foodCost.trim() === "" ? null : Number(values.foodCost) || 0,
      commission: values.commission.trim() === "" ? null : Number(values.commission) || 0,
      reviews: Number(values.reviews) || 0,
      target_aov: Number(values.targetAov) || 32,
    };
    const { data, error } = await supabase
      .from("daily_operating_data")
      ...
      .select("id,user_id,record_date,revenue,orders,food_cost,commission,reviews,target_aov")
      .single();
    if (error) throw new Error("经营数据保存失败：" + error.message);
    setDailyHistory(prev => [data as DailyRecord, ...prev.filter(r => r.record_date !== recordDate)].slice(0, 30));
  }

  function updateDailyReport(key: keyof DailyReport, value: string) {
    // 移动端输入框不要在 onChange 时做正则拦截，否则 iPad/iPhone 数字键盘可能出现“光标能点但数字进不去”。
    setDailyReport(v => ({ ...v, [key]: value }));
    setDailyReportResult(null);
  }

  async function generateDailyReport() {
    if (!dailyReport.revenue || !dailyReport.orders || dailyReportLoading) return;
    const { data: { session: currentSession } } = await supabase.auth.getSession();
    if (!currentSession) { setShowAuth(true); return; }
    if (remaining === 0) { setShowPro(true); return; }

    const revenue = Number(dailyReport.revenue) || 0;
    const orders = Number(dailyReport.orders) || 0;
    const foodCost = Number(dailyReport.foodCost) || 0;
    const commission = Number(dailyReport.commission) || 0;
    const reviews = Number(dailyReport.reviews) || 0;
    const targetAov = Number(dailyReport.targetAov) || 32;
    const aov = orders > 0 ? revenue / orders : 0;
    const hasProfitData = dailyReport.foodCost.trim() !== "" && dailyReport.commission.trim() !== "";
    ...
    const opportunity = Math.max(0, targetAov - aov) * orders;
    ...
    let status = "经营基本稳定";
    let issue = "今天没有明显异常，重点寻找客单价和利润提升机会";
    if (grossMargin !== null && grossMargin < 40) { status = "利润承压"; issue = "毛利率偏低，成本正在明显侵蚀利润"; }
    else if (reviewRate > 3) { status = "口碑预警"; issue = "差评率偏高，需要优先处理影响复购的问题"; }
    else if (aov > 0 && aov < targetAov) { status = "客单价偏低"; issue = "订单有了，但客单价还有提升空间"; }
    else if (grossMargin !== null && grossMargin >= 55) { status = "经营健康"; issue = "利润基础不错，可以把精力放在增量订单和复购上"; }

    const result: DailyReportResult = { revenue, orders, aov, grossProfit, grossMargin, reviewRate, opportunity, score, status, issue };
    setDailyReportResult(result);
    try {
      await saveDailyReport(currentSession, dailyReport);
    } catch (e: any) {
      console.error(e);
      setDailyReportAnswer(e?.message || "经营数据保存失败，但本次计算仍可继续。");
    }
    setDailyReportAnswer("");
    setDailyReportLoading(true);

    const prompt = `你是餐谋AI的餐饮经营日报助手。请根据老板今天提供的真实数据，生成一份能直接执行的《今日经营日报》。绝对不要编造数据、同比增长或不存在的顾客反馈；没有提供的数据就明确写“未提供”。

今日营业额：¥${revenue.toFixed(2)}
今日订单：${orders}
客单价：¥${aov.toFixed(2)}
目标客单价：¥${targetAov.toFixed(2)}
食材成本：${dailyReport.foodCost ? `¥${foodCost.toFixed(2)}` : "未提供"}
平台佣金：${dailyReport.commission ? `¥${commission.toFixed(2)}` : "未提供"}
差评数：${reviews}
差评率：${reviewRate.toFixed(2)}%
${grossMargin !== null ? `毛利：¥${(grossProfit as number).toFixed(2)}，毛利率：${grossMargin.toFixed(1)}%` : "毛利率：未提供，不能计算"}
系统初步判断：${issue}
理论客单价提升空间：¥${opportunity.toFixed(2)}（仅估算，不代表保证收入）

请严格按以下结构：
【老板一句话】一句话总结今天经营状态
【今天做得好的】最多2条，只能根据数据判断
【最大问题】指出今天最值得处理的1个问题，并解释原因
【钱从哪里来】给出1个最现实的增收机会，必须说明是估算还是策略建议
【今天3个动作】按优先级列3个具体动作，老板今天就能执行
【明天看3个数字】列出3个最重要的指标，并说明为什么
【本周方向】一句话告诉老板接下来一周最应该盯什么
要求：中文、短句、直接、具体，避免鸡汤和空话。`;

    try {
      const res = await fetch("/api/ai", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${currentSession.access_token}` },
        body: JSON.stringify({ tool: "ceo", message: prompt })
      });
      const data = await res.json().catch(() => ({}));
      if (res.status === 401) { await supabase.auth.signOut(); setSession(null); setShowAuth(true); throw new Error("登录已失效，请重新登录。"); }
      if (res.status === 402) { setRemaining(0); setShowPro(true); return; }
      if (!res.ok) throw new Error(data?.error || "日报生成失败，请稍后再试。");
      setDailyReportAnswer(data?.answer || "AI没有返回有效内容。");
      if (typeof data?.remaining === "number") setRemaining(data.remaining); else await loadCredits(currentSession);
    } catch (e: any) {
      setDailyReportAnswer(e?.message || "请求失败，请稍后再试。");
    } finally {
  ...
  function updateDiag(key: keyof Diag, value: string) {
    // 只负责接收输入；计算时再用 Number() 转换，避免移动端输入法拦截字符。
...
要求：中文、数字优先、短句、可执行，不假装知道真实损耗率。`;
  ...
  const latestHistory = dailyHistory[0];
  const historyGrossProfit = latestHistory && latestHistory.food_cost !== null && latestHistory.commission !== null
    ? latestHistory.revenue - latestHistory.food_cost - latestHistory.commission
    : null;
  const historyAov = latestHistory && latestHistory.orders > 0 ? latestHistory.revenue / latestHistory.orders : 0;
  const dashboardRevenue = dailyReportResult ? money(dailyReportResult.revenue) : latestHistory ? money(latestHistory.revenue) : "¥0";
  const dashboardOrders = dailyReportResult ? dailyReportResult.orders.toLocaleString("zh-CN") : latestHistory ? latestHistory.orders.toLocaleString("zh-CN") : "0";
  const dashboardAov = dailyReportResult ? money(dailyReportResult.aov) : latestHistory ? money(historyAov) : "¥0";
  const dashboardGrossProfit = dailyReportResult ? money(dailyReportResult.grossProfit) : latestHistory ? money(historyGrossProfit) : "—";
  const dashboardLabel = dailyReportResult || latestHistory ? "真实数据" : "等待录入";

  const last7 = [...dailyHistory].slice(0, 7).reverse();
  const sevenRevenue = last7.reduce((sum, r) => sum + Number(r.revenue || 0), 0);
  const sevenOrders = last7.reduce((sum, r) => sum + Number(r.orders || 0), 0);
  const sevenGrossProfit = last7.reduce((sum, r) => {
    if (r.food_cost == null || r.commission == null) return sum;
    return sum + Number(r.revenue || 0) - Number(r.food_cost || 0) - Number(r.commission || 0);
  }, 0);
  const sevenAov = sevenOrders > 0 ? sevenRevenue / sevenOrders : 0;
  const maxDayRevenue = Math.max(...last7.map(r => Number(r.revenue || 0)), 1);
  const dashboardIssue = latestHistory
    ? (historyGrossProfit === null ? "还缺食材成本和平台佣金，暂时无法准确判断利润" : historyGrossProfit < latestHistory.revenue * 0.35 ? "毛利偏低，先检查食材成本和平台佣金" : latestHistory.reviews >= 5 ? "差评偏多，优先处理最近差评" : "今天整体经营正常，重点提升客单价和复购")
    : "还没有经营数据，先录入今天的数据";
  const dashboardOpportunity = latestHistory && historyGrossProfit !== null
    ? Math.max(0, latestHistory.revenue * 0.4 - historyGrossProfit)
    : 0;
  const dashboardActions = latestHistory
    ? [
        historyGrossProfit === null ? "补充食材成本和平台佣金" : historyGrossProfit < latestHistory.revenue * 0.35 ? "检查高成本菜品，优先降损" : "保住高毛利爆款，继续推高销量",
        latestHistory.reviews >= 3 ? "今天把最近差评逐条处理" : "继续关注差评和出餐体验",
        latestHistory.orders > 0 && historyAov < Number(latestHistory.target_aov || 32) ? "通过套餐/加购把客单价拉到目标" : "设计一个提高复购的小活动"
      ]
    : ["先录入今日经营数据", "补齐食材成本和平台佣金", "再生成AI经营日报"];

  const proModalStyles = `
    .proOverlay{position:fixed!important;inset:0!important;z-index:9999!important;display:grid!important;place-items:center!important;padding:20px!important;background:rgba(2,10,7,.68)!important;backdrop-filter:blur(8px)!important;-webkit-backdrop-filter:blur(8px)!important;box-sizing:border-box!important;}
    .proOverlay .proModal{position:relative!important;width:min(460px,calc(100vw - 32px))!important;max-width:460px!important;max-height:calc(100vh - 32px)!important;overflow:auto!important;box-sizing:border-box!important;padding:30px!important;border-radius:26px!important;background:linear-gradient(160deg,#10261c 0%,#07130f 100%)!important;border:1px solid rgba(157,255,201,.18)!important;box-shadow:0 28px 80px rgba(0,0,0,.45)!important;color:#f4faf7!important;}
    .proOverlay .proClose{position:absolute!important;right:14px!important;top:14px!important;width:36px!important;height:36px!important;border:1px solid rgba(255,255,255,.12)!important;border-radius:11px!important;background:rgba(255,255,255,.06)!important;color:#dce8e2!important;display:grid!important;place-items:center!important;cursor:pointer!important;z-index:2!important;}
    .proOverlay .proIcon{width:52px!important;height:52px!important;border-radius:16px!important;display:grid!important;place-items:center!important;background:rgba(157,255,201,.12)!important;color:#9dffc9!important;margin-bottom:16px!important;}
    .proOverlay h2{margin:0!important;font-size:28px!important;line-height:1.2!important;letter-spacing:-.6px!important;color:#fff!important;}
    .proOverlay .proSubtitle{margin:8px 0 18px!important;color:#9fb1a8!important;font-size:14px!important;}
    .proOverlay .proPrice{font-size:42px!important;font-weight:900!important;line-height:1!important;color:#fff!important;margin:8px 0 22px!important;}
    .proOverlay .proPrice span{font-size:20px!important;margin-right:3px!important;}
    .proOverlay .proPrice small{font-size:14px!important;font-weight:600!important;color:#9fb1a8!important;margin-left:4px!important;}
    .proOverlay .proFeatures{display:grid!important;gap:10px!important;padding:16px!important;border-radius:16px!important;background:rgba(255,255,255,.045)!important;border:1px solid rgba(255,255,255,.07)!important;margin-bottom:18px!important;}
    .proOverlay .proFeatures>div{display:flex!important;align-items:center!important;gap:9px!important;color:#dce8e2!important;font-size:13px!important;line-height:1.4!important;}
    .proOverlay .proFeatures svg{color:#9dffc9!important;flex:none!important;}
    .proOverlay .paymentTitle{font-size:13px!important;font-weight:800!important;color:#dce8e2!important;margin-bottom:10px!important;}
    .proOverlay .paymentMethods{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;margin-bottom:14px!important;}
    .proOverlay .paymentMethod{min-height:56px!important;border:1px solid rgba(255,255,255,.12)!important;border-radius:14px!important;background:rgba(255,255,255,.045)!important;color:#dce8e2!important;cursor:pointer!important;font-size:14px!important;font-weight:800!important;}
    .proOverlay .paymentMethod.active{border-color:#9dffc9!important;background:rgba(157,255,201,.10)!important;color:#9dffc9!important;box-shadow:0 0 0 1px rgba(157,255,201,.12) inset!important;}
    .proOverlay .paymentNotice{display:flex!important;gap:9px!important;align-items:flex-start!important;padding:12px!important;border-radius:13px!important;background:rgba(157,255,201,.06)!important;color:#9fb1a8!important;font-size:12px!important;line-height:1.6!important;margin-bottom:14px!important;}
    .proOverlay .paymentNotice svg{color:#9dffc9!important;flex:none!important;margin-top:2px!important;}
    .proOverlay .proSubmit{width:100%!important;min-height:50px!important;border:0!important;border-radius:14px!important;background:#9dffc9!important;color:#082018!important;font-size:14px!important;font-weight:900!important;cursor:pointer!important;box-shadow:0 10px 24px rgba(80,220,145,.16)!important;}
    .proOverlay .proSubmit:disabled{opacity:.6!important;cursor:not-allowed!important;}
    .proOverlay .proError{margin-bottom:12px!important;padding:11px!important;border-radius:12px!important;background:rgba(255,80,80,.1)!important;color:#ff9a9a!important;font-size:12px!important;}
    .proOverlay .proSuccess{text-align:center!important;color:#fff!important;}
    .proOverlay .successIcon{width:64px!important;height:64px!important;margin:0 auto 16px!important;border-radius:50%!important;display:grid!important;place-items:center!important;background:#9dffc9!important;color:#082018!important;}
    @media (max-width:600px){.proOverlay{padding:12px!important;}.proOverlay .proModal{width:min(100%,430px)!important;max-height:calc(100vh - 24px)!important;padding:24px 20px!important;border-radius:22px!important;}.proOverlay h2{font-size:24px!important;}.proOverlay .proPrice{font-size:36px!important;}}
  `;

  return <main className="app">
    <style dangerouslySetInnerHTML={{__html: proModalStyles}} />
    <aside className="sidebar">
      ...
      <nav>{tools.map((tool) => { const Icon = tool.icon; return <button key={tool.id} type="button" className={active === tool.id ? "sideItem active" : "sideItem"} onClick={() => { setActive(tool.id); if (tool.id === "menu") document.getElementById("menu-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "ceo") document.getElementById("daily-report")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "review") document.getElementById("review-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "profit") document.getElementById("profit-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "marketing") document.getElementById("marketing-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "inventory") document.getElementById("inventory-ai")?.scrollIntoView({behavior:"smooth"}); else document.getElementById("ai")?.scrollIntoView({behavior:"smooth"}); }}><Icon size={18}/><span>{tool.name}</span>{active === tool.id && <i/>}</button>; })}</nav>
      <div className="sideBottom">
        <div className="proCard" style={{position:"relative",overflow:"hidden",padding:18,borderRadius:18,background:"linear-gradient(145deg,#102b20 0%,#0a1b14 100%)",border:"1px solid rgba(145,255,201,.18)",boxShadow:"0 12px 30px rgba(0,0,0,.18)",color:"#fff"}}>
          <div style={{position:"absolute",width:100,height:100,borderRadius:"50%",background:"rgba(132,255,190,.10)",right:-42,top:-42}}/>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative"}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}><div style={{width:32,height:32,borderRadius:10,display:"grid",placeItems:"center",background:"rgba(155,255,202,.12)",border:"1px solid rgba(155,255,202,.16)"}}><Sparkles size={16}/></div><span style={{fontSize:11,fontWeight:800,letterSpacing:1.4,color:"#9dffc9"}}>PRO</span></div>
            <span style={{fontSize:10,padding:"4px 7px",borderRadius:999,background:"rgba(255,255,255,.07)",color:"#b9c9c1"}}>¥29.9/月</span>
          </div>
          <strong style={{display:"block",marginTop:14,fontSize:18,letterSpacing:-.3}}>{remaining === -1 ? "PRO会员" : "升级餐谋AI PRO"}</strong>
          <p style={{margin:"6px 0 12px",fontSize:12,lineHeight:1.6,color:"#9fb1a8"}}>{remaining === -1 ? "全部AI经营能力已解锁" : "让AI每天帮你盯住利润和经营问题"}</p>
          {remaining !== -1 && <div style={{display:"grid",gap:7,marginBottom:14}}><div style={{fontSize:11,color:"#c5d2cc"}}>✓ 无限AI经营分析</div><div style={{fontSize:11,color:"#c5d2cc"}}>✓ 历史数据与经营复盘</div><div style={{fontSize:11,color:"#c5d2cc"}}>✓ 菜单 / 利润 / 营销 / 库存AI</div></div>}
          <button type="button" onClick={() => window.location.href="/pro"} style={{width:"100%",height:40,border:0,borderRadius:11,background:"#9dffc9",color:"#082018",fontSize:13,fontWeight:800,cursor:"pointer",boxShadow:"0 6px 18px rgba(80,220,145,.16)"}}>{remaining === -1 ? "查看会员权益 →" : "立即升级 PRO →"}</button>
        </div>
    ...
    <section className="content">
      <header className="topbar"><div><div className="breadcrumb">我的餐厅 / 经营驾驶舱</div><h1>早上好，老板 👋</h1></div><div className="topActions"><button type="button" className="iconButton"><Bell size={19}/></button><button type="button" className="dateButton">2026年9月12日</button>{!authLoading && (session ? <button type="button" className="accountButton" onClick={logout}><span>{avatarLetter}</span>退出</button> : <button type="button" className="loginButton" onClick={() => setShowAuth(true)}>登录 / 注册</button>)}</div></header>

      <section className="heroDashboard"><div className="heroText"><div className="status"><span/>DeepSeek AI 实时分析</div><h2>今天的生意，<br/><em>AI帮你盯着。</em></h2><p>从营业额、菜单、利润到差评，餐谋AI每天帮你发现经营机会。</p><button type="button" className="heroButton" onClick={() => document.getElementById("daily-report")?.scrollIntoView({behavior:"smooth"})}>开始今日经营日报 <ArrowUpRight size={17}/></button></div><div className="heroImage"><img src="https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=1200&q=90" alt="餐厅"/><div className="imageOverlay"><span>今日经营状态</span><strong>{diagResult ? (diagResult.score >= 80 ? "良好" : diagResult.score >= 60 ? "需要关注" : "需要干预") : "待体检"}</strong><small>{diagResult ? `AI经营评分 ${diagResult.score}` : "输入今日数据开始分析"}</small></div></div></section>

      <section className="stats"><Stat title="今日营业额" value={dashboardRevenue} change={dashboardLabel} icon={<Coins/>}/><Stat title="今日订单" value={dashboardOrders} change={dashboardLabel} icon={<ShoppingBag/>}/><Stat title="客单价" value={dashboardAov} change={dashboardLabel} icon={<BarChart3/>}/><Stat title="预计毛利" value={dashboardGrossProfit} change={dashboardLabel} icon={<TrendingUp/>}/></section>

      <section className="panel" id="owner-dashboard" style={{marginTop:18,color:"#14201b"}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start",marginBottom:18,flexWrap:"wrap"}}>
          <div><div style={{fontSize:11,fontWeight:800,letterSpacing:1.5,color:"#71847b"}}>OWNER DASHBOARD</div><h3 style={{margin:"6px 0 4px",fontSize:22}}>老板驾驶舱</h3><p style={{margin:0,color:"#71847b",fontSize:13}}>打开就看懂：今天赚多少、问题在哪、下一步做什么。</p></div>
          <button type="button" className="heroButton" style={{padding:"10px 14px",fontSize:13}} onClick={() => document.getElementById("daily-report")?.scrollIntoView({behavior:"smooth"})}>录入今日数据 <ArrowUpRight size={15}/></button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96,color:"#14201b"}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🔴 最大问题</div><strong style={{color:"#14201b"}}>{dashboardIssue}</strong></div>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96,color:"#14201b"}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>💰 机会空间</div><strong style={{color:"#14201b"}}>{dashboardOpportunity > 0 ? `约 ¥${dashboardOpportunity.toFixed(0)} 毛利空间` : "先补齐成本数据"}</strong></div>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96,color:"#14201b"}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🤖 AI老板建议</div><strong style={{color:"#14201b"}}>{dailyReportAnswer ? dailyReportAnswer.slice(0,90) : "生成今日经营日报后，这里会显示AI建议"}</strong></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:16,marginTop:16}}>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#f9fbfa"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><strong>近7天营业额</strong><span style={{fontSize:12,color:"#71847b"}}>{last7.length ? `累计 ¥${sevenRevenue.toLocaleString("zh-CN",{maximumFractionDigits:0})}` : "暂无数据"}</span></div>
            {last7.length ? <div style={{display:"flex",alignItems:"flex-end",gap:8,height:150}}>{last7.map((r) => { const value=Number(r.revenue||0); const h=Math.max(8,Math.round(value/maxDayRevenue*120)); return <div key={r.id} style={{flex:1,height:"100%",display:"flex",flexDirection:"column",justifyContent:"flex-end",alignItems:"center",gap:5}}><div title={`¥${value.toFixed(0)}`} style={{width:"100%",maxWidth:38,height:h,borderRadius:"7px 7px 3px 3px",background:"#173b2f"}}/><span style={{fontSize:10,color:"#71847b"}}>{String(r.record_date).slice(5)}</span></div>; })}</div> : <div style={{height:150,display:"grid",placeItems:"center",color:"#9aa9a2",fontSize:13}}>录入至少一天数据后显示趋势</div>}
          </div>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#f9fbfa",color:"#14201b"}}><strong style={{color:"#14201b"}}>7天经营概览</strong><div style={{display:"grid",gap:12,marginTop:16}}><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>营业额</span><b>{money(sevenRevenue)}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>订单</span><b>{sevenOrders.toLocaleString("zh-CN")}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>平均客单价</span><b>{money(sevenAov)}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>可计算毛利</span><b>{sevenGrossProfit ? money(sevenGrossProfit) : "—"}</b></div></div></div>
        </div>
        <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#f9fbfa",marginTop:16}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🎯 今天就做这3件事</div><div style={{display:"grid",gap:8,marginTop:10}}>{dashboardActions.map((a,i)=><div key={i} style={{display:"flex",gap:10,alignItems:"center",fontSize:14}}><span style={{width:24,height:24,borderRadius:8,background:"#eaf3ee",display:"grid",placeItems:"center",fontWeight:800,color:"#173b2f"}}>{i+1}</span><span style={{color:"#14201b"}}>{a}</span></div>)}</div></div>
      </section>

       <section className="panel" id="daily-report" style={{marginBottom:24,padding:24,borderRadius:22}}>
         <div className="panelHeader">
           <div><span>AI DAILY REPORT</span><h3>AI经营日报 · 今天到底赚得怎么样？</h3><p style={{margin:"6px 0 0",color:"#71847b"}}>每天填一次真实数据，餐谋AI帮你做老板最该看的经营总结。</p></div>
           <BarChart3 size={24}/>
         </div>
         <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:14,marginTop:20}}>
           <DiagInput label="今日营业额" value={dailyReport.revenue} onChange={v=>updateDailyReport("revenue",v)} placeholder="例如 8620" suffix="元"/>
           <DiagInput label="今日订单" value={dailyReport.orders} onChange={v=>updateDailyReport("orders",v)} placeholder="例如 386" suffix="单"/>
           <DiagInput label="目标客单价" value={dailyReport.targetAov} onChange={v=>updateDailyReport("targetAov",v)} placeholder="例如 32" suffix="元"/>
           <DiagInput label="食材成本" value={dailyReport.foodCost} onChange={v=>updateDailyReport("foodCost",v)} placeholder="例如 2600" suffix="元"/>
           <DiagInput label="平台佣金" value={dailyReport.commission} onChange={v=>updateDailyReport("commission",v)} placeholder="例如 800" suffix="元"/>
           <DiagInput label="差评数" value={dailyReport.reviews} onChange={v=>updateDailyReport("reviews",v)} placeholder="例如 3" suffix="条"/>
         </div>
         <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,marginTop:20,flexWrap:"wrap"}}>
           <div style={{fontSize:13,color:"#71847b"}}>日报会结合客单价、毛利率、差评率判断优先级 · 每次生成消耗1次AI额度</div>
           <button type="button" className="fullButton" onClick={generateDailyReport} disabled={dailyReportLoading || !dailyReport.revenue || !dailyReport.orders} style={{maxWidth:270,opacity:dailyReportLoading || !dailyReport.revenue || !dailyReport.orders ? .55 : 1}}>
             {dailyReportLoading ? "AI正在生成日报..." : "生成今日经营日报"}<ArrowUpRight size={16}/>
           </button>
         </div>
       </section>

       {dailyReportResult && <section style={{marginBottom:24}}>
         <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:14}}>
           <MetricCard label="经营评分" value={`${dailyReportResult.score}`} suffix="100分制"/>
           <MetricCard label="今日客单价" value={money(dailyReportResult.aov)} suffix={`目标 ${money(Number(dailyReport.targetAov) || 32)}`}/>
           <MetricCard label="毛利率" value={dailyReportResult.grossMargin === null ? "—" : `${dailyReportResult.grossMargin.toFixed(1)}%`} suffix={dailyReportResult.grossProfit === null ? "补充成本数据后计算" : money(dailyReportResult.grossProfit) + " 毛利"}/>
           <MetricCard label="经营状态" value={dailyReportResult.status} suffix={`差评率 ${dailyReportResult.reviewRate.toFixed(1)}%`} emphasis/>
         </div>
         <div className="panel" style={{marginTop:14,padding:24,borderRadius:22}}>
           <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
             <div style={{width:38,height:38,borderRadius:12,display:"grid",placeItems:"center",background:"#edf7f1"}}><Bot size={19}/></div>
             <div><div style={{fontSize:12,letterSpacing:1.2,color:"#71847b"}}>TODAY'S MANAGEMENT SUMMARY</div><h3 style={{margin:2}}>老板今天最该知道的事</h3></div>
           </div>
           <div style={{display:"grid",gridTemplateColumns:"1.1fr 1fr 1fr",gap:12,marginBottom:16}}>
             <ActionCard n="01" text={dailyReportResult.issue}/>
             <ActionCard n="02" text={dailyReportResult.opportunity > 0 ? `客单价若提升到目标，理论增收空间约 ${money(dailyReportResult.opportunity)}，这是估算，不是收入保证。` : "当前客单价已达到目标附近，优先把订单量和复购做上去。"}/>
             <ActionCard n="03" text={dailyReportResult.grossMargin !== null && dailyReportResult.grossMargin < 40 ? "先查食材成本、平台扣点和高损耗菜品，别急着加大促销。" : dailyReportResult.reviewRate > 3 ? "先处理高频差评原因，避免继续损失复购和转化。" : "今天重点记录订单、客单价和毛利，连续7天再判断趋势。"} />
           </div>
           {dailyReportAnswer && <div className="aiAnswer">
             <div className="answerTitle"><Bot size={18}/>餐谋AI · 今日经营日报</div>
             <div className="answerBody" style={{whiteSpace:"pre-wrap"}}>{dailyReportAnswer}</div>
           </div>}
         </div>
       </section>}


      <section className="panel" id="trend-center" style={{marginBottom:24,padding:24,borderRadius:22}}>
        <div className="panelHeader"><div><span>REAL DATA CENTER</span><h3>经营趋势 · 最近7天</h3><p style={{margin:"6px 0 0",color:"#71847b"}}>日报保存后自动形成趋势，连续记录比看单天数据更有价值。</p></div><BarChart3 size={24}/></div>
        {historyLoading ? <div style={{padding:"28px 0",color:"#71847b"}}>正在读取经营历史...</div> : dailyHistory.length === 0 ? (
          <div style={{marginTop:20,padding:24,borderRadius:16,background:"#f7faf8",border:"1px solid #e7eee9",color:"#71847b"}}>还没有历史数据。今天生成一次日报，数据就会自动保存。</div>
        ) : (
          <div style={{marginTop:20}}>
            {(() => {
              ...
              const avgAov = totalOrders > 0 ? totalRevenue / totalOrders : 0;
              const profits = rows.filter(r => r.food_cost !== null && r.commission !== null).map(r => r.revenue - (r.food_cost || 0) - (r.commission || 0));
              const totalProfit = profits.length ? profits.reduce((s,v)=>s+v,0) : null;
              const first = rows[0];
              const last = rows[rows.length-1];
              const revenueChange = first && first.revenue > 0 ? ((last.revenue-first.revenue)/first.revenue)*100 : null;
              return <>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:14,marginBottom:20}}>
                  <MetricCard label="7天营业额" value={money(totalRevenue)} suffix={`${rows.length}天已记录`}/>
                  <MetricCard label="7天订单" value={totalOrders.toLocaleString("zh-CN")} suffix="累计订单"/>
                  <MetricCard label="平均客单价" value={money(avgAov)} suffix="按7天累计计算"/>
                  <MetricCard label="7天毛利" value={money(totalProfit)} suffix={revenueChange === null ? "成本数据不足" : `首尾营业额 ${revenueChange >= 0 ? "+" : ""}${revenueChange.toFixed(1)}%`} emphasis/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(7,minmax(55px,1fr))",gap:10,alignItems:"end",height:190}}>
                  {rows.map(r => <div key={r.record_date} style={{height:"100%",display:"flex",flexDirection:"column",justifyContent:"flex-end",alignItems:"center",gap:7}}>
                    <strong style={{fontSize:11,color:"#3e5149"}}>{money(r.revenue)}</strong>
                    <div title={`${r.record_date} 营业额 ${money(r.revenue)}`} style={{width:"100%",maxWidth:58,height:`${Math.max(10,(r.revenue/maxRevenue)*120)}px`,borderRadius:"9px 9px 4px 4px",background:"linear-gradient(180deg,#79d6a7,#bdebd0)"}}/>
                    <span style={{fontSize:10,color:"#8a9a93"}}>{r.record_date.slice(5)}</span>
                  </div>)}
                </div>
              </>;
            })()}
      ...
      <section className="panel" id="ai-review" style={{marginBottom:24,padding:24,borderRadius:22}}>
        <div className="panelHeader"><div><span>AI WEEKLY REVIEW</span><h3>AI自动复盘 · 这7天到底怎么样？</h3><p style={{margin:"6px 0 0",color:"#71847b"}}>把最近7天真实数据交给AI，自动找变化、问题和下一步动作。</p></div><Sparkles size={24}/></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:14,marginTop:20}}>
          <MetricCard label="复盘天数" value={`${Math.min(7,dailyHistory.length)} 天`} suffix={dailyHistory.length ? "按最近记录计算" : "暂无数据"}/>
          <MetricCard label="7天营业额" value={money(sevenRevenue)} suffix="累计"/>
          <MetricCard label="7天订单" value={sevenOrders.toLocaleString("zh-CN")} suffix="累计"/>
          <MetricCard label="平均客单价" value={money(sevenAov)} suffix="按累计数据计算" emphasis/>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,marginTop:20,flexWrap:"wrap"}}>
          <div style={{fontSize:13,color:"#71847b"}}>AI只基于你已经保存的真实经营数据复盘 · 每次复盘消耗1次AI额度</div>
          <button type="button" className="fullButton" onClick={reviewLast7Days} disabled={review7Loading || dailyHistory.length===0} style={{maxWidth:250,opacity:review7Loading || dailyHistory.length===0 ? .55 : 1}}>{review7Loading ? "AI正在复盘..." : "开始7天AI复盘"}<ArrowUpRight size={16}/></button>
        </div>
        {review7Answer && <div className="aiAnswer" style={{marginTop:18}}><div className="answerTitle"><Bot size={18}/>餐谋AI · 7天经营复盘</div><div className="answerBody" style={{whiteSpace:"pre-wrap"}}>{review7Answer}</div></div>}
      </section>
...
function DiagInput({label,value,onChange,placeholder,suffix}:{label:string;value:string;onChange:(v:string)=>void;placeholder:string;suffix:string}) { return <label style={{display:"block"}}><span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:7,color:"#3e5149"}}>{label}</span><div style={{display:"flex",alignItems:"center",border:"1px solid #dce6e1",borderRadius:12,background:"#fff",padding:"0 12px"}}><input type="text" inputMode="decimal" autoComplete="off" value={value} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder} style={{width:"100%",height:46,border:0,outline:0,background:"transparent",fontSize:15}}/><span style={{fontSize:12,color:"#8a9a93"}}>{suffix}</span></div></label>; }
