...
要求：中文、数字优先、短句、可执行，不假装知道真实损耗率。`;
  ...
  return <main className="app">
    ...
    <section className="content">
      ...
      <section className="panel" id="history-center" style={{marginBottom:24,padding:24,borderRadius:22}}>
        <div className="panelHeader">
          <div>
            <span>OPERATING HISTORY</span>
            <h3>经营历史 · 最近30天</h3>
            <p style={{margin:"6px 0 0",color:"#71847b"}}>每天保存一次真实经营数据，餐谋AI自动帮你看趋势。</p>
          </div>
          <BarChart3 size={24}/>
        </div>
        {dailyHistory.length === 0 ? (
          <div style={{marginTop:20,padding:24,borderRadius:16,background:"#f7faf8",border:"1px solid #e7eee9",color:"#71847b"}}>
            暂无经营记录。先生成今天的经营日报。
          </div>
        ) : (
          <div style={{marginTop:20}}>
            {(() => {
              const rows = [...dailyHistory].sort((a,b) => b.record_date.localeCompare(a.record_date));
              ...
              const totalProfit = profitRows.reduce((sum,r) => sum + Number(r.revenue || 0) - Number(r.food_cost || 0) - Number(r.commission || 0), 0);
              const latest = rows[0];
              const previous = rows[1];
              const change = latest && previous && Number(previous.revenue) > 0
                ? ((Number(latest.revenue) - Number(previous.revenue)) / Number(previous.revenue)) * 100
                : null;
              return <>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:14,marginBottom:20}}>
                  <MetricCard label="30天营业额" value={money(totalRevenue)} suffix={`${rows.length}天已记录`}/>
                  <MetricCard label="30天订单" value={totalOrders.toLocaleString("zh-CN")} suffix={totalOrders > 0 ? `平均每天 ${(totalOrders/rows.length).toFixed(0)} 单` : "—"}/>
                  <MetricCard label="可计算毛利" value={profitRows.length ? money(totalProfit) : "—"} suffix={`${profitRows.length}天有成本数据`} emphasis/>
                </div>
                <div style={{overflowX:"auto",border:"1px solid #e5ece8",borderRadius:14}}>
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:620,fontSize:13}}>
                    <thead>
                      <tr style={{background:"#f7faf8",textAlign:"left"}}>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>日期</th>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>营业额</th>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>订单</th>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>客单价</th>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>毛利</th>
                        <th style={{padding:"12px 14px",color:"#71847b",fontWeight:700}}>差评</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rows.slice(0,30).map((r,i) => {
                        const rOrders = Number(r.orders || 0);
                        const rRevenue = Number(r.revenue || 0);
                        const rProfit = r.food_cost !== null && r.commission !== null
                          ? rRevenue - Number(r.food_cost || 0) - Number(r.commission || 0)
                          : null;
                        return <tr key={r.id || r.record_date} style={{borderTop:"1px solid #edf1ef",background:i===0?"#fbfefc":"#fff"}}>
                          <td style={{padding:"13px 14px",fontWeight:i===0?800:600}}>{r.record_date}{i===0 && <span style={{marginLeft:7,fontSize:11,padding:"3px 6px",borderRadius:999,background:"#e9f8f0",color:"#1f7a4d"}}>最新</span>}</td>
                          <td style={{padding:"13px 14px",fontWeight:750}}>¥{rRevenue.toLocaleString("zh-CN",{maximumFractionDigits:0})}</td>
                          <td style={{padding:"13px 14px"}}>{rOrders.toLocaleString("zh-CN")}</td>
                          <td style={{padding:"13px 14px"}}>{rOrders > 0 ? `¥${(rRevenue/rOrders).toFixed(2)}` : "—"}</td>
                          <td style={{padding:"13px 14px",fontWeight:700}}>{rProfit === null ? "—" : `¥${rProfit.toLocaleString("zh-CN",{maximumFractionDigits:0})}`}</td>
                          <td style={{padding:"13px 14px",color:Number(r.reviews||0)>3?"#b42318":"#3e5149"}}>{Number(r.reviews||0)}</td>
                        </tr>;
                      })}
                    </tbody>
                  </table>
                </div>
                <div style={{marginTop:14,fontSize:12,color:"#71847b"}}>
                  {change === null ? "继续记录数据后，会自动显示日环比。" : <>最近一天营业额较前一天 <strong style={{color:change>=0?"#1f7a4d":"#b42318"}}>{change>=0?"上涨":"下降"} {Math.abs(change).toFixed(1)}%</strong>。</>}
                </div>
              </>;
            })()}
          </div>
