...
export default function Home() {
  ...
  const localCalc = useMemo(() => {
    ...
    const hasProfitData =
      diag.foodCost.trim() !== "" &&
      diag.commission.trim() !== "";

    const grossProfit = hasProfitData
      ? revenue - foodCost - commission
      : null;

    const grossMargin =
      hasProfitData && revenue > 0
        ? ((grossProfit as number) / revenue) * 100
        : null;
    ...
    if (
      ...
      issue =
        "差评率偏高，正在影响复购与平台转化";
      priority = "优先处理差评与服务问题";
    } else if (
      aov > 0 &&
      aov < targetAov
    ) {
      issue =
        "客单价偏低，同样的订单量还有明显增收空间";
      priority = "优化套餐与加购";
    }
...
理论客单价增收空间：
¥${c.opportunity.toFixed(2)}

注意：
这个增收空间只是数学估算，不代表保证收入。
        ...
        {diagResult && (
          ...
          >
            ...
            >
              ...
              <MetricCard
                label="今日客单价"
                value={`¥${diagResult.aov.toFixed(
                  2
                )}`}
                suffix={
                  diagResult.aovGap >
                  0
                    ? `差 ¥${diagResult.aovGap.toFixed(
                        2
                      )}`
                    : "达标"
                }
              ...
              <MetricCard
                label="预计毛利"
                value={money(
                  diagResult.grossProfit
                )}
                suffix={
                  diagResult.grossMargin ===
                  null
                    ? "数据不足"
                    : `${diagResult.grossMargin.toFixed(
                        1
                      )}%`
                }
              />
              ...
              >
                <ActionCard
                  ...
                  text={
                    diagResult.aovGap >
                    0
                      ? `把套餐/加购设计到 ¥${diagResult.targetAov} 左右，先提高客单价。`
                      : "继续保持当前客单价，同时测试高毛利加购。"
                  }
                />
                ...
                <ActionCard
                  n="02"
                  text={
                    diagResult.reviewRate >
                    3
                      ? "逐条处理高频差评原因，优先解决重复出现的问题。"
                      : "检查高毛利菜品曝光，增加套餐和加购入口。"
                  }
                />
                ...
                <ActionCard
                  ...
                  text={
                    ...
                    diagResult.grossMargin <
                      40
                      ? "逐项核对食材成本和平台扣点，先砍掉低毛利损耗。"
                      : "明天继续记录营业额、订单、客单价，验证今天的调整。"
                  }
                />
              </div>
            </div>
