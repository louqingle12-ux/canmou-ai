...
要求：中文、数字优先、短句、可执行，不假装知道真实损耗率。`;
  ...
  return <main className="app">
    ...
    <section className="content">
      ...
      <section className="panel" id="owner-dashboard" style={{marginTop:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🔴 最大问题</div><strong>{dashboardIssue}</strong></div>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>💰 机会空间</div><strong>{dashboardOpportunity > 0 ? `约 ¥${dashboardOpportunity.toFixed(0)} 毛利空间` : "先补齐成本数据"}</strong></div>
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#fff",minHeight:96}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🤖 AI老板建议</div><strong>{dailyReportAnswer ? dailyReportAnswer.slice(0,90) : "生成今日经营日报后，这里会显示AI建议"}</strong></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:16,marginTop:16}}>
          ...
          <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#f9fbfa"}}><strong>7天经营概览</strong><div style={{display:"grid",gap:12,marginTop:16}}><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>营业额</span><b>{money(sevenRevenue)}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>订单</span><b>{sevenOrders.toLocaleString("zh-CN")}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>平均客单价</span><b>{money(sevenAov)}</b></div><div><span style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>可计算毛利</span><b>{sevenGrossProfit ? money(sevenGrossProfit) : "—"}</b></div></div></div>
        </div>
        <div style={{padding:16,border:"1px solid #e1ebe6",borderRadius:14,background:"#f9fbfa",marginTop:16}}><div style={{fontSize:12,color:"#71847b",marginBottom:7,display:"block"}}>🎯 今天就做这3件事</div><div style={{display:"grid",gap:8,marginTop:10}}>{dashboardActions.map((a,i)=><div key={i} style={{display:"flex",gap:10,alignItems:"center",fontSize:14}}><span style={{width:24,height:24,borderRadius:8,background:"#eaf3ee",display:"grid",placeItems:"center",fontWeight:800}}>{i+1}</span><span>{a}</span></div>)}</div></div>
      </section>
