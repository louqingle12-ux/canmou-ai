...
export default function Home() {
  ...
  const [reviewPlatform, setReviewPlatform] = useState("美团");
  const [reviewTone, setReviewTone] = useState("专业真诚");
  ...
  async function analyzeReview() {
    ...
    setReviewAnswer("");
    const prompt = `你是餐谋AI的外卖差评处理专家。请根据下面顾客真实差评，帮助餐饮老板处理问题。不要编造顾客没有说过的事实，不要攻击顾客，不要推卸责任。
...
请严格输出以下结构：
【建议回复】给顾客一段可以直接复制发送的回复，控制在120字以内，先表达感谢/歉意，再针对问题回应，最后给出具体改进或解决态度。不要出现“亲爱的顾客”等机械套话。
【差评原因】判断这条差评最可能对应的1-2个经营问题，只基于评论内容，不确定的地方明确说“不确定”。
【老板要改什么】给出3个具体动作，必须能落地，例如检查出餐、包装、配送、菜品标准或员工服务流程。
【优先级】高/中/低，并用一句话解释。
要求：中文、简洁、真诚、可直接执行；不要承诺无法确定的退款、赔偿或平台处理结果。`;
    try {
      ...
      setReviewAnswer(data?.answer || "AI没有返回有效内容。");
      if (typeof data?.remaining === "number") setRemaining(data.remaining); else await loadCredits(currentSession);
    } catch (e: any) { setReviewAnswer(e?.message || "请求失败，请稍后再试。"); }
  ...
  return <main className="app">
    <aside className="sidebar">
      ...
      <nav>{tools.map((tool) => { const Icon = tool.icon; return <button key={tool.id} type="button" className={active === tool.id ? "sideItem active" : "sideItem"} onClick={() => { setActive(tool.id); if (tool.id === "menu") document.getElementById("menu-ai")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "ceo") document.getElementById("diagnosis")?.scrollIntoView({behavior:"smooth"}); else if (tool.id === "review") document.getElementById("review-ai")?.scrollIntoView({behavior:"smooth"}); else document.getElementById("ai")?.scrollIntoView({behavior:"smooth"}); }}><Icon size={18}/><span>{tool.name}</span>{active === tool.id && <i/>}</button>; })}</nav>
    ...
    <section className="content">
      ...
      <section className="panel" id="review-ai" style={{marginBottom:24,padding:24,borderRadius:22}}>
        <div className="panelHeader"><div><span>REVIEW RECOVERY</span><h3>差评AI · 一条差评，直接告诉你怎么处理</h3><p style={{margin:"6px 0 0",color:"#71847b"}}>粘贴顾客差评，自动生成可直接回复的话术，并找出店铺真正需要整改的问题。</p></div><MessageSquareWarning size={24}/></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:20}}>
          <label style={{display:"block"}}><span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:7,color:"#3e5149"}}>平台</span><select value={reviewPlatform} onChange={e=>setReviewPlatform(e.target.value)} style={{width:"100%",height:46,border:"1px solid #dce6e1",borderRadius:12,padding:"0 12px",background:"#fff",color:"#14201b",fontSize:14}}><option>美团</option><option>饿了么</option><option>抖音</option><option>大众点评</option><option>其他</option></select></label>
          <label style={{display:"block"}}><span style={{display:"block",fontSize:13,fontWeight:700,marginBottom:7,color:"#3e5149"}}>回复语气</span><select value={reviewTone} onChange={e=>setReviewTone(e.target.value)} style={{width:"100%",height:46,border:"1px solid #dce6e1",borderRadius:12,padding:"0 12px",background:"#fff",color:"#14201b",fontSize:14}}><option>专业真诚</option><option>温和有温度</option><option>简洁利落</option></select></label>
        </div>
        <textarea value={reviewText} onChange={e=>setReviewText(e.target.value)} maxLength={2000} placeholder="例如：等了快一个小时，送来的时候汤都洒了，鸡肉也很少，以后不会再点了……" style={{width:"100%",minHeight:150,marginTop:12,resize:"vertical",border:"1px solid #dce6e1",borderRadius:14,padding:14,outline:0,fontSize:14,lineHeight:1.7,background:"#fff",color:"#14201b"}}/>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginTop:12,flexWrap:"wrap"}}>
          <span style={{fontSize:13,color:"#71847b"}}>{reviewText.length}/2000 · 每次分析消耗1次AI额度</span>
          <button type="button" className="fullButton" onClick={analyzeReview} disabled={reviewLoading || !reviewText.trim()} style={{maxWidth:240,opacity:reviewLoading || !reviewText.trim() ? .55 : 1}}>{reviewLoading?"AI正在处理差评...":"开始差评AI处理"}<ArrowUpRight size={16}/></button>
        </div>
        {reviewAnswer && <div className="aiAnswer" style={{marginTop:16}}><div className="answerTitle"><MessageSquareWarning size={18}/>餐谋AI差评处理结果</div><div className="answerBody" style={{whiteSpace:"pre-wrap"}}>{reviewAnswer}</div></div>}
      </section>
